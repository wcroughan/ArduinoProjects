...
long home_coord_x = 0;
long home_coord_y = 0;
long goal_coord_x = home_coord_x;
long goal_coord_y = home_coord_y;
long last_goal_x = 0;
long last_goal_y = 0;
...
AccelStepper motor2 = AccelStepper(AccelStepper::FULL4WIRE, 4, 5);
AccelStepper motor3 = AccelStepper(AccelStepper::FULL4WIRE, 6, 7);
AccelStepper motor4 = AccelStepper(AccelStepper::FULL4WIRE, 8, 9);

const int m1x = 0;
const int m1y = 0;
const int m2x = 0; //TODO
const int m2y = 0;
const int m3x = 0;
const int m3y = 0;
const int m4x = 0;
...
void loop()
{
    ...
    while (Serial.available())
    {
        if (waiting_on_x)
        {
            ...
            goal_coord_x = atoi(s_in.c_str());
            waiting_on_y = true;
        }
        else if (waiting_on_y)
        {
            String s_in = Serial.readString();
            goal_coord_y = atoi(s_in.c_str());
